...
let parse_card json_card =
  ...
  | str ->
    Scanf.sscanf str "Reg_Card { suit : %s@, rank : %s }" (fun suit_str rank_str ->
      let suit = 
        match suit_str with
        | "Spades" -> Spades
        | "Diamonds" -> Diamonds
...
(*return a list of squares representing the specified column.*)
  ...
  let extract_diag2 (b : t) =
    List.mapi (fun i row -> 
      List.nth row (List.length row - 1 - i)) b
...
let col_win = List.exists (fun i -> 
  check_line_for_win (extract_col b i)) 
  (List.init (List.length (List.hd b)) (fun x -> x)) in
